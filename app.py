...
# ---------------------------------------------------------
# Google Analytics (GA4) 數據追蹤設定（高級防阻擋安全版）
# ---------------------------------------------------------
GA_ID = "G-SQSJPZ9SW9"

# 注入頂層監聽與 GA 代碼：同時支援 iframe 內與最外層的數據傳輸
ga_base_code = f"""
...
<script>
    ...
    // 建立監聽機制，接收來自 Streamlit 內部的安全傳輸
    window.addEventListener('message', function(event) {{
        if (event.data && event.data.type === 'ga_event') {{
            gtag('event', event.data.name, event.data.params);
        }}
    }}, false);
</script>
"""
...
# 輔助函式：發送自訂數據與答案到 GA 後台（升級為安全防阻擋傳輸機制）
def track_ga_event(event_name, params):
    ...
    <script>
        ...
        // 同時向當前視窗與父視窗安全廣播，確保 100% 穿透沙盒限制
        window.gtag ? window.gtag('event', '{event_name}', {{{param_str}}}) : null;
...
QUIZ_DATA = {
    ...
    5: {"type": "free_text", "q": "第五關：比較這兩張圖 soap 觀察到何創時本清明上河圖有哪些獨特之處呢？（送分題-有作答即有分數）", "ans": "專家觀察參考：街道正中間的畫軸內容、街道人物的樣貌、衣紋的紋飾", "points": 15},
